...
public class Leiho3DeptKudeaketa_Update extends JFrame {
...
//	final static Logger logger = Logger.getLogger(Leiho2LangileKudeaketa.class);
	...
	public Leiho3DeptKudeaketa_Update(final Departamentua departamentua) {
		...
		txtDeptKod.setEnabled(false);
		txtDeptKod.setText(departamentua.getDepart_kod());
		...
		txtIzena.setText(departamentua.getIzena());
...
//					logger.info("Departamentu izena lekuan: " + e + " sartu du.");
		...
		jcbKokapena.setSelectedItem(departamentua.getKokapena());
		...
		jcbEraikuntzaZbk.setSelectedItem(departamentua.getEraikuntza_zbk());
		...
		txtIrakKop.setText(departamentua.getIrakasle_kop());
		...
		btnGorde.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {				
				...
				if (!txtIzena.getText().matches("[a-zA-Z]") && txtIrakKop.getText().matches("[0-9]{1,3}")) {
					kontrolatzailea.MetodoakBBDD.deptTaulaAldatu(departamentua);
					dispose();
				}				
			}
		});
